...
class BasePage:
    ...
    def is_element_usable(self, handler, assertion=True, **kwargs):
        method_name = repr(handler).split('.')[1].split()[0]
        page_class = repr(handler).split('.pages.')[-1].split()[0].split('.')[-1]
        log.info(f'Check if element "{page_class}.{method_name}()" is usable')
        try:
            element = handler(**kwargs)
            if not element.is_usable():
                if assertion:
                    assert False, f'Element "{page_class}.{method_name}()" is unusable'
                return False
            return True
        except TimeoutException:
            if assertion:
    ...
    def validate_elements_usable(self, element_handlers, assertion=True, **kwargs):
        for handler in element_handlers:
            if not self.is_element_usable(handler, assertion, **kwargs) and not assertion:
                return False
        if not assertion:
            return True
